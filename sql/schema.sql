-- CloudOps Cost & Reliability Guardrails (V1)
-- Postgres schema: source of truth for state + history.
...
-- Helpful for last-30-days charts and service breakdowns
...
CREATE TABLE IF NOT EXISTS reliability_metrics (
  ...
  unit        TEXT NOT NULL, -- e.g. 'count', 'ms'
...
CREATE TABLE IF NOT EXISTS alerts (
  ...
  source           TEXT NOT NULL, -- e.g. 'cost_explorer', 'cloudwatch', 'anomaly_detection_job'

  detected_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  window_start     TIMESTAMPTZ NULL,
  window_end       TIMESTAMPTZ NULL,

  -- Explainability fields (nullable because cost and reliability alerts differ)
  service_name     TEXT NULL,  -- for cost alerts
  metric_name      TEXT NULL,  -- for reliability alerts ('5xx_count', 'p95_latency_ms')

  observed_value   NUMERIC(16, 6) NULL,
  baseline_value   NUMERIC(16, 6) NULL,
  threshold_value  NUMERIC(16, 6) NULL,

  rule_name        TEXT NOT NULL, -- simple string like 'daily_service_cost_threshold'
...
-- Common UI queries: open alerts, latest first
...
CREATE TABLE IF NOT EXISTS recommendations (
  ...
  owner        TEXT NULL, -- optional in V1 (can be email)
...
CREATE INDEX IF NOT EXISTS idx_recos_project_status_updated
  ON recommendations (project_id, status, updated_at DESC);
...
CREATE TABLE IF NOT EXISTS events (
  ...
  entity_id    UUID NULL, -- for 'job' events this may be NULL
